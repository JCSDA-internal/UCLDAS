...
use LND_domains, only : LND_domain_type, get_domain_extent, compute_block_extent
use LND_domains, only : get_global_shape, get_domain_extent_dsamp2
use LND_error_handler, only : LND_error, LND_mesg, FATAL
...
public LND_grid_init, LND_grid_end, set_derived_metrics, set_first_direction
...
!> Ocean grid type. See lnd_grid for details.
type, public :: ocean_grid_type
  type(LND_domain_type), pointer :: Domain => NULL() !< Ocean model domain
  type(LND_domain_type), pointer :: Domain_aux => NULL() !< A non-symmetric auxiliary domain type.
...
!> LND_grid_init initializes the ocean grid array sizes and grid memory.
subroutine LND_grid_init(G, param_file, US, HI, global_indexing, bathymetry_at_vel)
...
# include "version_variable.h"
  ...
  character(len=40)  :: mod_nm  = "LND_grid" ! This module's name.
  ...
  call LND_mesg("  LND_grid.F90, LND_grid_init: allocating metrics", 5)
...
! setup block indices.
  ...
  if (nblocks < 1) call LND_error(FATAL, "LND_grid_init: " // &
  ...
  do i = 1, niblock-1
    if (iend(i)-ibegin(i) > iend(niblock)-ibegin(niblock) ) call LND_error(FATAL, &
       "LND_grid_init: the last block size in x-direction is not the largest")
  enddo
  do j = 1, njblock-1
    if (jend(j)-jbegin(j) > jend(njblock)-jbegin(njblock) ) call LND_error(FATAL, &
       "LND_grid_init: the last block size in y-direction is not the largest")
  enddo
  ...
  if ( G%block(nblocks)%ied+G%block(nblocks)%idg_offset > G%HI%ied + G%HI%idg_offset ) &
        call LND_error(FATAL, "LND_grid_init: G%ied_bk > G%ied")
  if ( G%block(nblocks)%jed+G%block(nblocks)%jdg_offset > G%HI%jed + G%HI%jdg_offset ) &
        call LND_error(FATAL, "LND_grid_init: G%jed_bk > G%jed")
...
subroutine rescale_grid_bathymetry(G, m_in_new_units)
  ...
  if (m_in_new_units < 0.0) &
    call LND_error(FATAL, "rescale_grid_bathymetry: Negative depth units are not permitted.")
  if (m_in_new_units == 0.0) &
    call LND_error(FATAL, "rescale_grid_bathymetry: Zero depth units are not permitted.")
...
subroutine allocate_metrics(G)
  ...
  ALLOC_(G%orog_filt(isd:ied,jsd:jed))   ; G%orog_filt(:,:) = 0.0
  ...
  ALLOC_(G%lsmask(isd:ied,jsd:jed))       ; G%lsmask(:,:) = 0.0
  ...
  allocate(G%gridLonLnd(isg:ieg))   ; G%gridLonLnd(:) = 0.0
  allocate(G%gridLonB(G%IsgB:G%IegB)) ; G%gridLonB(:) = 0.0
  allocate(G%gridLatT(jsg:jeg))   ; G%gridLatT(:) = 0.0
  allocate(G%gridLatLnd(jsg:jeg))   ; G%gridLatLnd(:) = 0.0
...
subroutine LND_grid_end(G)
  ...
  deallocate(G%gridLonLnd) ; deallocate(G%gridLatLnd)
  deallocate(G%gridLonB) ; deallocate(G%gridLatB)

  deallocate(G%Domain%mpp_domain)
  deallocate(G%Domain)

end subroutine LND_grid_end
