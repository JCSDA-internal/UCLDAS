...
type, public, extends(ucldas_fields) :: ucldas_state

contains

  ! constructors / destructors
  procedure :: create => ucldas_state_create

  ! interactions with increment
  procedure :: diff_incr=> ucldas_state_diff_incr
  procedure :: add_incr => ucldas_state_add_incr

  ! misc
  procedure :: rotate => ucldas_state_rotate
  procedure :: convert => ucldas_state_convert
  procedure :: logexpon => ucldas_state_logexpon
...
!------------------------------------------------------------------------------
subroutine ucldas_state_create(self, geom, vars)
  class(ucldas_state),         intent(inout) :: self
  ...
  call self%ucldas_fields%create(geom, vars)
end subroutine ucldas_state_create


! ------------------------------------------------------------------------------
!> Rotate horizontal vector
subroutine ucldas_state_rotate(self, coordinate, uvars, vvars)
  class(ucldas_state),  intent(inout) :: self
  ...
  integer :: z, i
  type(ucldas_field), pointer :: uocn, vocn
...
end subroutine ucldas_state_rotate


! ------------------------------------------------------------------------------
!> add a set of increments to the set of fields
subroutine ucldas_state_add_incr(self, rhs)
  class(ucldas_state),  intent(inout) :: self
  class(ucldas_increment), intent(in) :: rhs

  type(ucldas_field), pointer :: fld, fld_r
  integer :: i, k
...
end subroutine ucldas_state_add_incr


! ------------------------------------------------------------------------------
!> subtract two sets of fields, saving the results separately
subroutine ucldas_state_diff_incr(x1, x2, inc)
  class(ucldas_state),      intent(in)    :: x1
  class(ucldas_state),      intent(in)    :: x2
  class(ucldas_increment), intent(inout)  :: inc

  integer :: i
  type(ucldas_field), pointer :: f1, f2
...
end subroutine ucldas_state_diff_incr

! ------------------------------------------------------------------------------
!> ConvertState app
subroutine ucldas_state_convert(self, rhs)
  class(ucldas_state), intent(inout) :: self ! target
  class(ucldas_state), intent(in)   :: rhs   ! source
  integer :: n
  type(ucldas_convertstate_type) :: convert_state
  type(ucldas_field), pointer :: field1, field2, hocn1, hocn2, layer_depth
...
end subroutine ucldas_state_logexpon
! ------------------------------------------------------------------------------
