...
namespace ucldas {
  ...
  Geometry::Geometry(const eckit::Configuration & conf,
    ...
    ucldas_geo_set_atlas_lonlat_f90(keyGeom_, atlasFieldSet_->get());
    ...
    // Fill ATLAS fieldset
    atlasFieldSet_.reset(new atlas::FieldSet());
    ucldas_geo_fill_atlas_fieldset_f90(keyGeom_, atlasFieldSet_->get());
  }
  // -----------------------------------------------------------------------------
  ...
  // -----------------------------------------------------------------------------
  Geometry::~Geometry() {
    ucldas_geo_delete_f90(keyGeom_);
  }
  // -----------------------------------------------------------------------------
  void Geometry::gridgen() const {
    ucldas_geo_gridgen_f90(keyGeom_);
  }
  // -----------------------------------------------------------------------------
  GeometryIterator Geometry::begin() const {
    // return start of the geometry on this mpi tile
    int ist, iend, jst, jend;
    ucldas_geo_start_end_f90(keyGeom_, ist, iend, jst, jend);
