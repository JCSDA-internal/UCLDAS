...
subroutine ucldas_getvalues_fillgeovals(self, geom, fld, t1, t2, locs, geovals)
  ...
  do ivar = 1, geovals%nvar
    ...
    case default
      call fckit_log%debug("ucldas_interpfields_mod:interp geoval does not exist")
    end select
...
subroutine ucldas_getvalues_fillgeovals_ad(self, geom, incr, t1, t2, locs, geovals)
  ...
  do ivar = 1, geovals%nvar
    ...
    if (.not. found) then
      ...
      case default
        call abor1_ftn("ucldas_interpfields_mod:getvalues_ad geoval does not exist")
...
function nlev_from_ufovar(fld, var) result(nval)
  ...
  case default
     nval = 0
     call fckit_log%debug("ucldas_interpfields_mod:nlef_from_ufovar geoval does not exist: "//var)
