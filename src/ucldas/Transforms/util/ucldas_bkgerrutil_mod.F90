! (C) Copyright 2017-2020 UCAR
!
! This software is licensed under the terms of the Apache Licence Version 2.0
! which can be obtained at http://www.apache.org/licenses/LICENSE-2.0.

module ucldas_bkgerrutil_mod

use fckit_configuration_module, only: fckit_configuration
use kinds, only: kind_real
use ucldas_fields_mod, only: ucldas_fields, ucldas_field
use ucldas_utils, only: ucldas_adjust

implicit none

private
public :: ucldas_bkgerr_bounds_type, &
          ucldas_bkgerr_readbounds, ucldas_bkgerr_applybounds

type :: ucldas_bkgerr_bounds_type
   real(kind=kind_real) :: t_min, t_max
   real(kind=kind_real) :: s_min, s_max
   real(kind=kind_real) :: ssh_min, ssh_max
   real(kind=kind_real) :: cicen_min, cicen_max
   real(kind=kind_real) :: hicen_min, hicen_max
 contains
   procedure :: read => ucldas_bkgerr_readbounds
   procedure :: apply => ucldas_bkgerr_applybounds
end type ucldas_bkgerr_bounds_type

contains

! ------------------------------------------------------------------------------
!> Read bounds from config
subroutine ucldas_bkgerr_readbounds(self, f_conf)
  class(ucldas_bkgerr_bounds_type), intent(inout) :: self
  type(fckit_configuration),      intent(in)    :: f_conf

  ! Get bounds from configuration
  call f_conf%get_or_die("t_min", self%t_min)
  call f_conf%get_or_die("t_max", self%t_max)
  call f_conf%get_or_die("s_min", self%s_min)
  call f_conf%get_or_die("s_max", self%s_max)
  call f_conf%get_or_die("ssh_min", self%ssh_min)
  call f_conf%get_or_die("ssh_max", self%ssh_max)
  call f_conf%get_or_die("cicen_min", self%cicen_min)
  call f_conf%get_or_die("cicen_max", self%cicen_max)
  call f_conf%get_or_die("hicen_min", self%hicen_min)
  call f_conf%get_or_die("hicen_max", self%hicen_max)

end subroutine ucldas_bkgerr_readbounds

! ------------------------------------------------------------------------------
!> Setup the static background error
subroutine ucldas_bkgerr_applybounds(self, fld)
  class(ucldas_bkgerr_bounds_type), intent(inout) :: self
  type(ucldas_fields),              intent(inout) :: fld

  type(ucldas_field), pointer :: field

  integer :: isc, iec, jsc, jec, i, j, n
  real(kind=kind_real) :: vmin, vmax

  ! Apply config bounds to background error
  isc = fld%geom%isc ; iec = fld%geom%iec
  jsc = fld%geom%jsc ; jec = fld%geom%jec

  do n=1,size(fld%fields)
    field => fld%fields(n)
    select case(field%name)
    case ("tocn")
      vmin = self%t_min
      vmax = self%t_max
    case ("socn")
      vmin = self%s_min
      vmax = self%s_max
    case ("ssh")
      vmin = self%ssh_min
      vmax = self%ssh_max
    case ("cicen")
      vmin = self%cicen_min
      vmax = self%cicen_max
    case ("hicen")
      vmin = self%hicen_min
      vmax = self%hicen_max
    case default
      cycle
    end select

    do i = isc, iec
      do j = jsc, jec
        field%val(i,j,:) = ucldas_adjust(field%val(i,j,:), vmin, vmax)
      end do
    end do
  end do

end subroutine ucldas_bkgerr_applybounds

end module ucldas_bkgerrutil_mod
