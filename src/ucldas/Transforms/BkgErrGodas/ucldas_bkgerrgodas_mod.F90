...
use ucldas_utils, only: ucldas_diff
use ucldas_bkgerrutil_mod, only: ucldas_bkgerr_bounds_type
use ucldas_omb_stats_mod, only: ucldas_omb_stats, ucldas_domain_indices

implicit none

private
public :: ucldas_bkgerrgodas_config, &
          ucldas_bkgerrgodas_setup, ucldas_bkgerrgodas_mult, &
          ucldas_bkgerrgodas_tocn, ucldas_bkgerrgodas_socn, &
          ucldas_bkgerrgodas_ssh

!> Fortran derived type to hold configuration D
type :: ucldas_bkgerrgodas_config
   type(ucldas_state),         pointer :: bkg
   type(ucldas_geom),          pointer :: geom
   type(ucldas_fields)                 :: std_bkgerr
   type(ucldas_bkgerr_bounds_type)     :: bounds         ! Bounds for bkgerrgodas
...
end type ucldas_bkgerrgodas_config
...
subroutine ucldas_bkgerrgodas_setup(f_conf, self, bkg, geom)
  type(fckit_configuration),        intent(in) :: f_conf
  type(ucldas_bkgerrgodas_config), intent(inout) :: self
  type(ucldas_state),         target, intent(in) :: bkg
  type(ucldas_geom),          target, intent(in) :: geom

  type(ucldas_field), pointer :: field, field_bkg
  integer :: i
  character(len=800) :: fname = 'ucldas_bkgerrgodas.nc'
  ...
  call ucldas_bkgerrgodas_tocn(self)
  call ucldas_bkgerrgodas_socn(self)
  call ucldas_bkgerrgodas_ssh(self)
...
end subroutine ucldas_bkgerrgodas_setup

! ------------------------------------------------------------------------------
!> Apply background error: dxm = D dxa
subroutine ucldas_bkgerrgodas_mult(self, dxa, dxm)
  type(ucldas_bkgerrgodas_config),  intent(in) :: self
  type(ucldas_increment),           intent(in) :: dxa
  type(ucldas_increment),        intent(inout) :: dxm

  type(ucldas_field), pointer :: field_m, field_e, field_a
...
end subroutine ucldas_bkgerrgodas_mult

! ------------------------------------------------------------------------------
!> Derive T background error from vertial gradient of temperature
subroutine ucldas_bkgerrgodas_tocn(self)
  type(ucldas_bkgerrgodas_config),     intent(inout) :: self
  ...
  type(ucldas_omb_stats) :: sst
  type(ucldas_field), pointer :: tocn_b, tocn_e, hocn, layer_depth
  ...
  do i = domain%is, domain%ie
     do j = domain%js, domain%je
        if (self%geom%mask2d(i,j).eq.1) then
           ...
           call ucldas_diff(sig1(:), tocn_b%val(i,j,:), hocn%val(i,j,:))
...
end subroutine ucldas_bkgerrgodas_tocn

! ------------------------------------------------------------------------------
!> Derive unbalanced SSH background error, based on latitude
subroutine ucldas_bkgerrgodas_ssh(self)
  type(ucldas_bkgerrgodas_config),     intent(inout) :: self
  type(ucldas_domain_indices), target :: domain
  integer :: i, j
...
end subroutine ucldas_bkgerrgodas_ssh


! ------------------------------------------------------------------------------
!> Derive unbalanced S background error, based on MLD
subroutine ucldas_bkgerrgodas_socn(self)
  type(ucldas_bkgerrgodas_config),     intent(inout) :: self
  !
  type(ucldas_domain_indices), target :: domain
  type(ucldas_field), pointer :: field, mld, layer_depth
  integer :: i, j, k
...
end subroutine ucldas_bkgerrgodas_socn

end module ucldas_bkgerrgodas_mod
