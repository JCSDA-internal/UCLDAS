...
subroutine ucldas_cov_setup(self, f_conf, geom, bkg, vars)
  ...
  if (init_ocean) then
     call ucldas_bump_correlation(self, self%ocean_conv(1), geom, f_conf, domain)
  end if
  ...
  if (init_seaice) then
     call ucldas_bump_correlation(self, self%seaice_conv(1), geom, f_conf, domain)
  end if
...
subroutine ucldas_bump_correlation(self, horiz_convol, geom, f_conf, domain)
  class(ucldas_cov),        intent(inout) :: self   !< The covariance structure
